...
public final class ServerObservable {
    ...
    private static Subscription closingSubscription(final ServerSocket serverSocket) {
    ...
    static Observable<Conversation> requests(Socket socket, InputStream is) {
        ...
        // aggregate the bytes of the header
                ...
                .groupBy(isHeader())
                // can cache this with very little cost because we are caching
                // two references to Observables only
                .cache();
        Observable<byte[]> header = grouped.flatMap(toHeader(true)).first();
        Observable<byte[]> messageBody = grouped.flatMap(toHeader(true));
        return Observable.just(new Conversation(new Request(header, messageBody), new Response(socket)));
    }

    private static Func1<byte[], Boolean> isHeader() {
    ...
    private static Func1<GroupedObservable<Boolean, byte[]>, Observable<byte[]>> toHeader(final boolean value) {
        return new Func1<GroupedObservable<Boolean, byte[]>, Observable<byte[]>>() {
            @Override
            public Observable<byte[]> call(GroupedObservable<Boolean, byte[]> group) {
     ...
     * @return
     */
    public static Observable<byte[]> aggregateHeader(Observable<byte[]> source, final byte[] pattern) {

        return source.lift(new OperatorAggregateToPattern(pattern));
    }

    public static class OperatorAggregateToPattern implements Operator<byte[], byte[]> {
        ...
        @Override
        public Subscriber<? super byte[]> call(final Subscriber<? super byte[]> child) {
    ...
    private static void doOnNext(final Subscriber<? super byte[]> child, byte[] bytes, AtomicBoolean found,
            ByteArrayOutputStream buffer, ByteMatcher splitMatcher) {
        if (found.get()) {
            child.onNext(bytes);
        } else {
            try {
                ...
                if (index != -1) {
                    ...
                    if (index + splitMatcher.patternLength() == array.length) {
                        ...
                        byte[] rest = Arrays.copyOfRange(array, index + splitMatcher.patternLength(), array.length);
                        child.onNext(header);
                        child.onNext(rest);
                    }
